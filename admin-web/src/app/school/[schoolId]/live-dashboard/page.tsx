...
export default function LiveDashboardPage({ params }: { params: Promise<{ schoolId: string }> }) {
  ...
  useEffect(() => {
    if (connected && activeTrips.length > 0) {
      console.log('[LiveDashboard] Joining bus rooms for', activeTrips.length, 'buses');
      activeTrips.forEach((trip) => {
        joinBusRoom(trip.bus.id);
      });
    }
  }, [connected, activeTrips, joinBusRoom]);
  ...
  const fetchActiveTrips = async () => {
    ...
    try {
      ...
      // Join bus rooms immediately if socket is connected
      if (connected && trips.length > 0) {
        console.log('[LiveDashboard] Socket connected, joining', trips.length, 'bus rooms');
        trips.forEach((trip) => {
          console.log('[LiveDashboard] Joining bus room for bus:', trip.bus.id, trip.bus.plateNumber);
          joinBusRoom(trip.bus.id);
        });
      }
